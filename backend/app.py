...
try:
    ...
    print("[OK] statsmodels 已成功安装并导入")
except Exception as e:
    STATSMODELS_AVAILABLE = False
    print(f"[警告] statsmodels 导入失败，将使用模拟预测。错误：{e}")
...
try:
    ...
    print("[警告] scipy 未安装，核密度估计将使用简化算法")
...
try:
    ...
    print("[警告] prophet 未安装，Prophet模型将使用模拟预测")
...
def load_data():
    ...
    try:
        ...
        data_url = os.environ.get('DATA_FILE_URL', 'https://github.com/6jmm2k8g4t-commits/false.github.io/releases/download/v1.0/earthquake_dataset.csv.gz')
        ...
        if not os.path.exists(data_file):
            print(f"[INFO] 数据文件不存在，正在从 GitHub 下载：{data_url}")
            need_download = True
        else:
            # 检查文件是否是 Git LFS 指针文件（很小）或空文件
            file_size = os.path.getsize(data_file)
            if file_size < 10000:  # 小于 10KB 可能是 LFS 指针
                print(f"[警告] 文件太小 ({file_size} bytes)，可能是 Git LFS 指针，重新下载...")
                need_download = True
            else:
                print(f"[OK] 使用现有数据文件：{data_file} ({file_size} bytes)")
        ...
        if need_download:
            ...
            print(f"[INFO] 开始下载数据文件...")
            
            # 使用浏览器 User-Agent 避免被 GitHub 限制
            headers = {
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
                'Accept': 'application/octet-stream'
            }
            
            response = requests.get(data_url, timeout=300, stream=True, headers=headers, allow_redirects=True)
            response.raise_for_status()
            
            # 检查文件大小
            content_length = int(response.headers.get('content-length', 0))
            print(f"  文件大小：{content_length} bytes ({content_length / (1024*1024):.1f} MB)")
            
            if content_length < 10000:
                print(f"  [警告] 文件太小，可能是 LFS 指针！检查 URL 是否正确")
                print(f"  响应内容前 200 字节：{response.content[:200]}")
                raise Exception(f"下载的文件太小 ({content_length} bytes)，可能是 Git LFS 指针文件")
            
            # 保存到临时文件
            temp_file = data_file + '.gz'
            ...
            with open(temp_file, 'wb') as f:
                for chunk in response.iter_content(chunk_size=8192):
                    ...
                    if downloaded % (10 * 1024 * 1024) == 0:  # 每 10MB 打印进度
                        print(f"  下载进度：{downloaded // (1024*1024)}MB / {total_size // (1024*1024)}MB")
            
            print(f"[OK] 压缩文件已下载：{temp_file} ({os.path.getsize(temp_file)} bytes)")
            ...
            # 删除临时文件
            os.remove(temp_file)
            print(f"[OK] 数据文件已解压到：{data_file} ({os.path.getsize(data_file)} bytes)")
        ...
        if os.path.exists(full_path):
            ...
            print(f"[INFO] CSV 文件列名：{list(temp_df.columns)}")
            ...
            print(f"[OK] 加载 {len(df)} 条数据")
            
            # 预计算高风险区域数据
            print("[INFO] 预计算高风险区域数据...")
            _high_risk_regions_cache = _compute_high_risk_regions()
            print("[OK] 高风险区域数据预计算完成")
        else:
            print("[警告] 数据文件不存在")
            df = None
    except Exception as e:
